...
function startApiServer() {
    ...
    // 生产模式下启动API服务器
    const apiPath = (0, path_1.join)(__dirname, '../api/server.js');
    apiServer = (0, child_process_1.spawn)('node', [apiPath], {
        stdio: 'inherit',
            ...
            submenu: [
                {
                    label: '关于芥子博客',
                    click: () => {
                        electron_1.shell.openExternal('https://github.com/your-username/jiezi-blog');
                    }
                }
            ]
        }
...
electron_1.ipcMain.handle('open-external', (event, url) => {
    electron_1.shell.openExternal(url);
});
